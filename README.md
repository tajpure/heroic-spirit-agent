...
内置 HSA 不是相关人物本人、意识复制品、数字替身或授权代表。`steve-jobs`、`charlie-munger` 和 `donella-meadows` 都采用 `inspired_synthesis`：输出必须区分事实、综合推断和猜测，不能伪造引语、私人记忆或人物对当代事件的“真实立场”。人物名气不增加投票权重。
...
`hsa doctor` 用于检查真实 Hermes、三个 Profile alias、Soul 指纹、记忆配置与本地 terminal backend。只准备运行 `demo` 时，Hermes 或 Profile 缺失是正常的；doctor 会展示诊断并以状态码 4 表示“尚未可运行真实后端”。
...
Hermes 与三个 HSA Profile 已通过 `hsa doctor` 后，启动真实会议：
...
- 聚焦产品体验：通常选择 Steve Jobs HSA 加一名风险挑战者；
- 聚焦资本和下行风险：通常选择 Charlie Munger 与 Donella Meadows HSA；
- 产品与系统交叉问题：通常选择 Steve Jobs 与 Donella Meadows HSA；
- 跨组织、资本、系统的长期战略：选择战略内阁，可按相关度保留两席或三席；
- 发布型或 `risk_tier=high`：强制使用完整红队，保留提案者、批评者和独立裁判；
- 没有足够信号：回退完整产品圆桌，不武断裁员。
...
当前内置三个基础组织。自动路由会在基础组织内生成仅对本次运行有效的 HSA 席位，组织 ID 仍保持稳定，以便共享记忆和审批不会因成员组合而碎片化：

| ID | 协议 | 用途 |
|---|---|---|
| `product-roundtable` | `roundtable` | 三个视角先独立判断，再阅读匿名摘要并修订评分 |
| `launch-red-team` | `red_team` | Jobs HSA 为蓝队/主席，Munger HSA 为红队，Meadows HSA 为独立裁判 |
| `strategy-cabinet` | `cabinet` | 产品、资本风险和系统外部性分工会商；主席 override 必须进入人工确认 |
...
该命令会修改 Hermes 的 Profile 目录，并基于默认 Profile 创建 `hsa-steve-jobs`、`hsa-charlie-munger` 和 `hsa-donella-meadows`。默认位置是 `~/.hermes/profiles/`；设置了 `HERMES_HOME` 时以该配置为准。Bootstrap 会写入版本化 `SOUL.md`，启用 Hermes 原生私有 memory 和 user profile，并设置：
...
Orchestrator 在每次调用前按交集计算 allowlist，只把获准 toolsets 交给对应 Profile；调用携带 `run_id`、`hsa_id` 和阶段元数据，enabled toolsets 与最终响应哈希进入审计。有实时订阅者时，兼容的 Hermes 通过 NDJSON bridge 返回可见响应增量，以及工具名称、调用 ID、参数/结果大小和哈希；无订阅者或 bridge 不兼容时仍使用 quiet CLI 的最终 stdout。当前两条路径都不导出完整 session transcript 或真实 tool artifact 内容，因此不能把 Hermes 内部工具调用说成已被本项目完整审计，也不能让模型凭空引用 artifact ID。
